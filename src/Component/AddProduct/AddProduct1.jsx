...
const AddProduct=()=>{
  ...
  const handleSelectedVoice = (voiceId) => {
      ...
      <div className="images-container">
        {/* <div className="thumbnail-images">
          {avatarImages.map((image, index) => (
            <div key={index}>
              <img src={image.thumbnail} alt={`Thumbnail Image ${index}`} onClick={() => handleImageSelect(image)} />
            </div>
          ))}
        </div> */}
    ...
    <div key={index}>
      ...
      {avatarImagesFetched && <AvatarVoices setSelectedVoice={handleSelectedVoice}/>}
      <button onClick={saveAvatarDetails} className="addproduct-btn" >Save Avatar Details</button>
        </div>
    );
};
